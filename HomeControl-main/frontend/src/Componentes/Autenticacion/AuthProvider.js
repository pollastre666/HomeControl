...
const checkPermissionAccess = (user, requiredPermissions) => {
  ...
  // Permitir acceso si el usuario es cliente
...
const checkDeviceAccess = async (user, deviceId) => {
  ...
  // Asegúrate de comparar con userId
...
export const AuthProvider = ({ children }) => {
  ...
  const signup = useCallback(async ({ email, password, displayName }) => {
    // Mejorar validación y mensaje de error
    if (!auth || !email || !password || !displayName) {
      console.error('Datos incompletos:', { email, password, displayName });
      showToast('Datos requeridos: nombre, correo y contraseña', 'error');
