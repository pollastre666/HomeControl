...
const LoginForm = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    password: '',
    confirmPassword: '',
    displayName: '', // Cambiar username por displayName
  });
  ...
  const validateForm = () => {
    ...
    if (isSignUp) {
      if (!formData.displayName || formData.displayName.length < 3) { // Cambiar username por displayName
    ...
    try {
      if (isSignUp) {
        await signup({
          ...
          displayName: formData.displayName, // Cambiar username por displayName
        });
        toast.success('Cuenta creada. Por favor, verifica tu correo.');
        setFormData({ email: '', password: '', confirmPassword: '', displayName: '' }); // Cambiar username por displayName
        setIsSignUp(false);
      } else {
